            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
						...
						<li class="breadcrumb-item"><?= _r('Gallery Management', 'Gallery Management'); ?></li>
                        <li class="breadcrumb-item">
                            <a href="<?php echo add_photo_gallery_url(); ?>">{{ menu }}</a>
                        </li>
                    </ol>
                </div>
            </div>
                    ...
                    <div style="width:100%; margin-top: 50px;">
                        <div class="row">
						  ...
						  <h4>
							...
							echo _r($first['gallery_name_en'], $first['gallery_name_th']); ;?>
						  </h4>
						...
						<?php endif; ?>
						  ...
						  <div class="column text-centered" v-for="(item, index) in gallery_photos" v-bind:key="item.id_gallery_photo">
							<div v-if="item.photo_url">

							<span>{{ <?= _r('item.photo_desc', 'item.photo_desc'); ?> }}</span>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
		<form>
		<input type="hidden" v-model="id_gallery_category">
		...
		<div class="input-group mb-3">
			...
			<input type="text" v-model="photo_desc" class="form-control" placeholder="Photo Description" required>
		</div>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
		<form>
		<input type="hidden" v-model="id_gallery_category">
		<input type="hidden" v-model="photo_row_update.id_gallery_photo">
		<div class="input-group mb-3">
			<div class="input-group-prepend">				
			  <span class="input-group-text"><font color="red">*</font>Sequence</span>
			</div>
		...
		<div class="input-group mb-3">
			...
			<input type="text" v-model="photo_row_update.photo_desc" class="form-control" placeholder="Photo Description" required>
		</div>
...
$(document).ready(function() {
    app = new Vue({
        ...
        data: {
			...
			gallery_photos: <?php echo json_encode($gallery_photos); ?>,
			photo_desc: '',
			first: <?php echo json_encode($first); ?>,
			id_gallery_category : "<?php echo json_encode($first['id_gallery_category']); ?>",
			file: "",
			fileUpdate: "",
        ...
        methods: {
			...
			uploadFile: function(){
			   ...
			   formData.append('photo_desc', this.photo_desc);
			   formData.append('id_gallery_category', this.id_gallery_category);
			   formData.append('file', this.file);
			   
			   axios.post('<?php echo save_gallery_photo_url(); ?>', formData,
			   {
				  headers: {
			 ...
			 updateFile: function(){
			   ...
			   formData.append('photo_desc', this.photo_row_update.photo_desc);
			   formData.append('id_gallery_photo', this.photo_row_update.id_gallery_photo);
			   formData.append('id_gallery_category', this.id_gallery_category);
			   formData.append('file', this.fileUpdate);
			   
			   axios.post('<?php echo update_gallery_photo_url(); ?>', formData,
			   {
				  headers: {
					'Content-Type': 'multipart/form-data'
				  }
			 ...
			 get_photo: function(id) {
				...
				let self = this;
                self.photo_row_update = JSON.parse(JSON.stringify(this.gallery_photos[id]));
			...
			delete_photo: function(id) {
				...
				var r = confirm ("Are you sure you want to delete this photo?");
				if (r) {
					let param = {'id_gallery_photo': selected.id_gallery_photo};
					console.log(param);
					
                    $.post("<?php echo delete_photo_url(); ?>", param, function(res) {
                        console.log(res);
						if (res.result == 'false') {
                            alert(res.message);
                            return;
