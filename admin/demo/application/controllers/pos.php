...
class Pos extends MY_Controller
{
	...
	public function edit_guest($id = '')
	{
		...
		$tmp['id_credit'] = '0';
		$tmp['credit_description'] = '-No Credit-';
	...
	public function save_guest()
	{
		...
		// Added by Mychelle - 24/10/2023
		$_POST['id_discount']  = ($_POST['id_discount'] > 0) ? $_POST['id_discount'] : null;
		$_POST['id_credit']  = ($_POST['id_credit'] > 0) ? $_POST['id_credit'] : null;
		/////
	...
	public function _getBooking($id_guest_info = 0, $project = 'All', $room = 'All', $booking_number = '', $booking_status = 'All', $date_type = 'Booked', $date_from = '', $date_to = '')
	{
		...
		foreach ($booking as $i => $b) {
			//echo "<br>".$b['booking_number'].' - '.$booking[$i]['booking_number']."<br>";
			...
			if ($project != 'All') {
				$this->db->join('room_details', 'booking_room.id_room_details = room_details.id_room_details', 'LEFT');
	...
	public function delete_booking()
	{
		...
		if (empty($_POST['id'])) {
			...
			if (count($booking_extra) > 0) {
				foreach ($booking_item as $b) {
					$this->db->where('id_booking_item', $b['id_booking_item']);
	...
	public function late_booking($id = '')
	{
		if (!has_permission('booking', 'view')) {
			header("Location: " . home_url());
		}
	...
	public function save_booking()
	{
		...
		// credit 
		if (empty($booking_info['id_credit'])) {
			$booking_info['id_credit'] = null;
			$booking_info['credit_due_date'] = null;
			$booking_info['credit_term'] = null;
			$booking_info['credit_description'] = null;
		}
	...
	public function save_late_booking()
	{
		header('Content-Type: application/json; charset=utf-8');
		$ret = array('result' => 'false', 'message' => '');
	...
	public function _getInvoice($id_booking = '')
	{
		...
		$pdf->Cell(19, 7, formatDate($booking['check_in_date']), 0, 0, 'R');
		...
		// 	$chunks = array();
		// 	$temp = array();
		// 	$words = explode(" ", $booking['guest_address']);
		// 	$_text = "";
		// 	foreach ($words as $key => $word) {
		// 		$_text .= $word;
		// 		$temp[] = $word;

		// 		if (strlen($_text) > $text_len) {
		// 			$chunks[] = implode(' ', $temp);
		// 			$temp = array();
		// 		}
		// 	}
		// 	if (array_filter($chunks)) {
		// 		foreach (array_filter($chunks) as $key => $val) {
		// 			$pdf->Cell(60, 7, iconv('UTF-8', 'TIS-620', $val));
		// 			$pdf->SetY($pdf->GetY() + 7);
		// 			$pdf->SetX(50);
		// 			array_push($stack_for_guest, 1);
		// 		}
		// 	} else {
		// 		$pdf->Cell(60, 7, iconv('UTF-8', 'TIS-620', $booking['guest_address']));
		// 		$pdf->SetY($pdf->GetY() + 7);
		// 		$pdf->SetX(50);
		// 		array_push($stack_for_guest, 1);
		// 	}
		...
		$pdf->Cell(19, 7, formatDate($booking['check_out_date']), 0, 0, 'R');
		if (isset($stack_for_guest) && $stack_for_guest) {
			for ($i = 0; $i < count($stack_for_guest); $i++) {
				$pdf->SetY($pdf->GetY() + 7);
			}
		...
		// $pdf->SetX(170);
		$pdf->Cell(19, 7, dateDiff($booking['check_in_date'], $booking['check_out_date']), 0, 0, 'R');

		//billing detail
		$pdf->SetY($pdf->GetY() + 7);
		$pdf->Cell(8, 0, '');
		$pdf->Cell(32, 7, _t('Bill to:', 'ชื่อใบเสร็จ:'));
		$pdf->Cell(60, 7, iconv('UTF-8', 'TIS-620', $booking['billing_name']));
		if ($booking['id_credit']) {
			$pdf->SetX(150);
			$pdf->Cell(30, 7, _t('Credit Term:', 'เครดิต:     '));
			$pdf->SetX(186);
			$pdf->Cell(19, 7, empty($booking['credit_term']) ? '-' : $booking['credit_term'] . " " . _t('Days', '    วัน'));
		}
		...
		if (isset($booking['billing_address']) && $booking['billing_address']) {
			$chunks_billing = array();
			$temp_billing = array();
			$words = explode(" ",  $booking['billing_address']);
			$_text = "";
			foreach ($words as $key => $word) {
				$_text .= $word;
				$temp_billing[] = $word;
				...
				if (strlen($_text) > $text_len) {
					$chunks_billing[] = implode(' ', $temp_billing);
					$temp_billing = array();
				}
			}
			if (array_filter($chunks_billing)) {
				foreach (array_filter($chunks_billing) as $key => $val) {
					$pdf->Cell(60, 7, iconv('UTF-8', 'TIS-620', $val));
					$pdf->SetY($pdf->GetY() + 7);
					$pdf->SetX(50);
				...
				if ($booking['id_credit']) {
					$pdf->SetY($pdf->GetY() - 7);
					$pdf->SetX(150);
					$pdf->Cell(30, 7, _t('Due date:', 'วันครบกำหนดชำระ:'));
					$pdf->Cell(19, 7, empty($booking['credit_due_date']) ? '-' : formatDate($booking['credit_due_date']), 0, 0, 'R');
				}
			} else {
				$pdf->Cell(60, 7, iconv('UTF-8', 'TIS-620', $booking['billing_address']));
				if ($booking['id_credit']) {
					$pdf->SetX(150);
					$pdf->Cell(30, 7, _t('Due date:', 'วันครบกำหนดชำระ:'));
					$pdf->Cell(19, 7, empty($booking['credit_due_date']) ? '-' : formatDate($booking['credit_due_date']), 0, 0, 'R');
		...
		// $pdf->SetY($pdf->GetY() + 7);
		// if (isset($stack_for_billing) && $stack_for_billing) {
		// 	for ($i = 0; $i < count($stack_for_billing); $i++) {
		...
		$pdf->SetY($pdf->GetY() - 7);
		$pdf->Cell(8, 0, '');
		$pdf->Cell(32, 7, _t('TAX ID:', 'เลขประจำตัวผู้เสียภาษี:'));
	...
	public function test_get_booking () {
		//public function _getBooking($id_guest_info = 0, $project = 'All', $room = 'All', $booking_number = '', $booking_status = 'All', $date_type = 'Booked', $date_from = '', $date_to = '')
		$result = $this->_getBooking(0, 1, $room = 'All', $booking_number = '', $booking_status = 'All', $date_type = 'Booked', $date_from = '2022-12-01', $date_to = '2023-10-18');
		echo "<br>RESULT";
		print_r($result);
		echo "<br>";
	}
