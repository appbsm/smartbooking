...
class Booking extends MY_Controller {
	...
	public function save_booking () {
		...
		//$booking_num = '';
		$check_in_date = date_reformat($this->input->post('h_check_in_date'), 'day_to_year_dash');
		$check_out_date = date_reformat($this->input->post('h_check_out_date'), 'day_to_year_dash');
		...
		// get discount
		$guest = $this->m_guest->get_profile_by_guestID($this->input->post('id_guest'));
		$this->load->model('m_discount');
		...
		if ($this->input->post('packages') != '') {
			foreach ($id_package as $pkg) {
				...
				foreach ($package_items as $i) {
					...
					// UPDATE CART
					if ($page == 'cart_items') {
						$this->m_cart->delete_to_cart_by_package ($i->id_package, $this->input->post('id_guest'), $p[2]);			
					}
				}
				$package_room_details[$p[0]] = $rd;
			}
		}
		//echo "Total_package_item_price: ".$total_package_item_price;
		//echo "<br><br>ROOOMS<br><br>";
		//print_r($rooms);
		...
		if ($this->input->post('rooms') != '') {
			...
			foreach ($rooms as $room) {
				...
				//$room_type_details = $this->m_room_type->get_room_by_type($arr_room_rate[0]);
				...
				*/
				...
				// UPDATE CART
				if ($page == 'cart_items') {
					$this->m_cart->delete_to_cart_by_room_type ($arr_room_rate[0], $this->input->post('id_guest'), $arr_room_rate[2]);			
				}
			}
		}
	...
	public function test_send_email () {
		...
		$this->_sendEmail($to = 'panya@buildersmart.com', $subject = 'Test', $message = 'Test', $attachment = '');
	}
