...
class Room_details extends CI_Controller {
	...
	public function index()
	{
		...
		if (!empty($_POST)) {
			...
			$data['id_room_type'] = $this->input->post('h_id_room_type');
			$data['page'] = 'room_details';
			$this->session->set_userdata('room_details', json_encode($data));
		}	 
		else {
			$data = (array)json_decode($this->session->userdata('room_details'));
		}
		//print_r($data);
		$id_room_type = $data['id_room_type'];
		if (isset($data['id_room_type']) || $data['id_room_type'] != '') {
	...
	function get_season_price () {
		/*$check_in_date = '2023-02-11';
		$check_out_date = '2023-02-14';
		$id_room_types = array(1, 2, 4);
		*/
		/*$in_date = explode('-', $this->input->post('check_in_date'));
		$check_in_date = $in_date[2].'-'.$in_date[1].'-'.$in_date[0];
		$out_date = explode('-', $this->input->post('check_out_date'));
		...
		$id_room_type = $this->input->post('id_room_type');			
		$dates = $this->date_array($check_in_date, $check_out_date);
		$id_room_types = explode(',', $this->input->post('id_room_type'));
		$dates = $this->date_array($check_in_date, $check_out_date);
		$room_price_date = array();			
			foreach ($id_room_types as $id_room_type) {
				...
				$room->room_type_name_en = $room_type->room_type_name_en;
				$room->price_date = $price_date;
				array_push($room_price_date, (array)$room);	
			}	    
		echo json_encode($room_price_date);
	}
	
	function test_get_season_price () {
		$check_in_date = '2023-02-11';
		$check_out_date = '2023-02-14';
		$id_room_types = array(1);
		...
		//if (!empty($_POST)) {
			...
			foreach ($id_room_types as $id_room_type) {
				...
				foreach  ($dates as $indx => $d) {
					...
					if ($indx == 0) {
						echo "1"."<br>";
						//$_price->id_room_type = $id_room_type;
						$_price->night_ctr = $night_ctr;
						$_price->unit_price = $rate;
						$_price->item_total_price = $rate*$night_ctr;
						array_push($price_date, (array)$_price);	
		...
		//print_r($price_date);
		foreach ($room_price_date as $key => $val) {
			echo "<br>----<br>".$key." - ";
			print_r($val);
		} 		
	...
	public function get_room_photos () {
		if (!empty($_POST)) {
			$id_room_type = $this->input->post('id_room_type');
			$room_type_photos = $this->m_room_type->get_room_type_photos_by_modular($id_room_type);
			echo json_encode($room_type_photos);
		}
	}
}
